...
def pd_to_list_pn_other(name_of_sheet):
	df = pd.read_excel('spare_part_list.xlsx', sheet_name=name_of_sheet)
	return df['Customer'].tolist(), df[['Project_no','City']].values.tolist(),\
	df['City'].tolist(), df[['Project_no','Customer']].values.tolist() #  df[['Project',City']].values.tolist(): Keyerror 

def pd_to_lists_agent(name_of_sheet):
	df = pd.read_excel('spare_part_list.xlsx', sheet_name=name_of_sheet)
	return df['Short Name'].tolist(), df[['Company','Person','Address','Telefone','E-Mail','City','Province']].values.tolist(),\
	df['City'].tolist(),df[['Short Name','Company','Person','Address','Telefone','E-Mail','Province']].values.tolist(),\
	df['Province'].tolist(), df[['Short Name','Company','Person','Address','Telefone','E-Mail','City']].values.tolist()
...
def lists_to_dict_m(l, ll):
	...
	for elem, elem_l in zip(l, ll):
		...
		if elem in result:
			...
			result[elem] = [elem_l] #no value hashed to the key yet
	return result
...
def search_pn():
	while True:
		str_input = input("Enter the project name or location or type q to return to the upper level memu, please:\n")
		print("\n")
		if str_input == "q":
			break
			...
			for elem in dict_name[str_input]:
				print(elem)
				print("\n")
		elif str_input in dict_location:
			for elem in dict_location[str_input]:
				print(elem)
				print("\n")
		else:
			print("It's not a Amepa Project or Amepa doesn't have any project in that location!")
		print("\n")	

def search_agent():
	while True:
		str_input = input("Enter the agent company short name or city or province or type q to return to the upper level memu,\
		 please:\n")	
		if str_input == "q":
			break
		elif str_input in dict_agent:
			print(dict_agent[str_input])
		elif str_input in dict_city:
			for elem in dict_city[str_input]:
				print(elem)
				print("\n")
		elif str_input in dict_province:
			for elem in dict_province[str_input]:
				print(elem)
				peint("\n")
		else:
			print("It's not a Amepa Agent!")
		print("\n")	
...
if __name__ == '__main__':
	...
	raw_project_list_tsd, location_list_tsd = pd_to_list_pn_sds("TSD-Projects")
	raw_project_list_esd, location_list_esd = pd_to_list_pn_sds("ESD-Projects")
	project_name_list_srm, ll_srm_name, project_loc_list_srm, ll_srm_loc = pd_to_list_pn_other("SRM-Projects")
	project_name_list_ofm, ll_ofm_name, project_loc_list_ofm, ll_ofm_loc = pd_to_list_pn_other("OFM-Projects")
	...
	ll_tsd_name = combine_to_ll(project_no_list_tsd, location_list_tsd)
	ll_tsd_loc = combine_to_ll(project_no_list_tsd, project_name_list_tsd)
	ll_esd_name = combine_to_ll(project_no_list_esd, location_list_esd)
	ll_esd_loc = combine_to_ll(project_no_list_esd, project_name_list_esd)

	all_name_l = add_4_lists(project_name_list_tsd, project_name_list_esd, project_name_list_srm, project_name_list_ofm)
	all_name_set = set(all_name_l)
	#for name in all_name_set:
		#print(name)
		#print("\n")
	all_loc_l = add_4_lists(location_list_tsd, location_list_esd, project_loc_list_srm, project_loc_list_ofm)
	all_name_ll = add_4_lists(ll_tsd_name, ll_esd_name, ll_srm_name, ll_ofm_name)
	all_loc_ll = add_4_lists(ll_tsd_loc, ll_esd_loc, ll_srm_loc, ll_ofm_loc)


	dict_name = lists_to_dict_m(all_name_l, all_name_ll)
	dict_location = lists_to_dict_m(all_loc_l, all_loc_ll)

	agent_l, info_ll, city_l, city_ll, province_l, province_ll = pd_to_lists_agent("Agents")
	dict_agent = lists_to_dict(agent_l, info_ll)
	dict_city = lists_to_dict_m(city_l, city_ll)
	dict_province = lists_to_dict_m(province_l, province_ll)
